...
import com.chuongvd.view.customcalendar.CalendarController;
...
public class MonthByWeekFragment extends SimpleDayPickerFragment
        implements OnScrollListener, OnTouchListener {
    private static final String TAG = "MonthFragment";
    private static final String TAG_EVENT_DIALOG = "event_dialog";

    private static MonthByWeekFragment sInstance;
    ...
    Runnable mLoadingRunnable = new Runnable() {
        ...
        public void run() {
            if (!mIsDetached) {
                ArrayList<Event> events = Event.fakeDumpListEvent();
                setEvent(events);

                //((MonthByWeekAdapter) mAdapter).setEvents(mFirstLoadedJulianDay, mLastLoadedJulianDay - mFirstLoadedJulianDay, );
                //mLoader = (CursorLoader) getLoaderManager().initLoader(0, null,
                //        MonthByWeekFragment.this);
            }
        }
    };
    private SimpleWeeksAdapter.OnDayTappedListener mOnDayTappedListener;
    ...
    protected void setUpAdapter() {
        ...
        if (mAdapter == null) {
            ...
            mAdapter.setOnDayTappedListener(mOnDayTappedListener);
    ...
    public static MonthByWeekFragment newInstance() {
        if (sInstance == null) {
            sInstance = new MonthByWeekFragment();
        }
        return sInstance;
    }
    ...
    protected void setUpHeader() {
        ...
        mDayLabels = Utils.getDaysOfWeek();
    }
    ...
    public void setEvent(ArrayList<Event> events) {
        ...
        if (child != null) {
            int julianDay = child.getFirstJulianDay();
            int week = Utils.getWeeksSinceEpochFromJulianDay(julianDay, Time.SUNDAY);
            mFirstLoadedJulianDay = Utils.getJulianMondayFromWeeksSinceEpoch(week) - 1;
        }
        // -1 to ensure we get all day events from any time zone
        mTempTime.setJulianDay(mFirstLoadedJulianDay - 1);
        mLastLoadedJulianDay = mFirstLoadedJulianDay + (mNumWeeks) * 7;
        // +1 to ensure we get all day events from any time zone
        mTempTime.setJulianDay(mLastLoadedJulianDay + 1);
        ((MonthByWeekAdapter) mAdapter).setEvents(mFirstLoadedJulianDay,
                mLastLoadedJulianDay - mFirstLoadedJulianDay, events);
    }
    ...
    //                Time.getJulianDay(mSelectedDay.toMillis(true), mSelectedDay.gmtoff) - (mNumWeeks
    //                        * 7 / 2);
    //        mEventUri = updateUri();
    ...
    //        Event.buildEventsFromCursor(events, data, mContext, mFirstLoadedJulianDay,
    //                mLastLoadedJulianDay);
    //        ((MonthByWeekAdapter) mAdapter).setEvents(mFirstLoadedJulianDay,
    ...
    public void setOnDayTappedListener(SimpleWeeksAdapter.OnDayTappedListener onDayTappedListener) {
        mOnDayTappedListener = onDayTappedListener;
        if (mAdapter != null) {
            mAdapter.setOnDayTappedListener(onDayTappedListener);
        }
    }
}
