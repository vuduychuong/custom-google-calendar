...
public class SimpleWeekView extends View {
    ...
    protected static float DAY_SEPARATOR_WIDTH = 1.5f;
    protected static int MINI_DAY_NUMBER_MARGIN_TOP = 18;
    ...
    protected int mFocusMonthSundayColor;
    protected int mOtherMonthSundayColor;
    protected int mFocusMonthSaturdayColor;
    protected int mOtherMonthSaturdayColor;
    ...
    public SimpleWeekView(Context context) {
        ...
        mFocusMonthSundayColor = res.getColor(R.color.month_mini_sunday_number);
        mOtherMonthSundayColor = res.getColor(R.color.month_other_month_sunday_number);
        mFocusMonthSaturdayColor = res.getColor(R.color.month_mini_saturday_number);
        mOtherMonthSaturdayColor = res.getColor(R.color.month_other_month_saturday_number);
        ...
        if (mScale == 0) {
            ...
            if (mScale != 1) {
                ...
                MIN_HEIGHT *= mScale;
                MINI_DAY_NUMBER_MARGIN_TOP *= mScale;
    ...
    public void setWeekParams(HashMap<String, Integer> params, String tz) {
        ...
        if (params.containsKey(VIEW_PARAMS_NUM_WEEKS)) {
            mNumWeeks = params.get(VIEW_PARAMS_NUM_WEEKS);
        }
        ...
        mToday = -1;
        int focusMonth =
                params.containsKey(VIEW_PARAMS_FOCUS_MONTH) ? params.get(VIEW_PARAMS_FOCUS_MONTH)
                        : DEFAULT_FOCUS_MONTH;
        for (; i < mNumCells; i++) {
            if (time.monthDay == 1) {
                mFirstMonth = time.month;
    ...
    protected void drawWeekNums(Canvas canvas) {
        int y = (int) (MINI_DAY_NUMBER_MARGIN_TOP + DAY_SEPARATOR_WIDTH);
        int nDays = mNumCells;
        int i = 0;
        int divisor = 2 * nDays;
        if (mShowWeekNum) {
        ...
        boolean isFocusMonth;
        //boolean isFocusMonth = mFocusDay[i];
        //mMonthNumPaint.setColor(isFocusMonth ? mFocusMonthColor : mOtherMonthColor);
        mMonthNumPaint.setFakeBoldText(false);
        for (; i < nDays; i++) {
            isFocusMonth = mFocusDay[i];
            if ((mShowWeekNum && i == 1) || (!mShowWeekNum && i == 0)) {
                mMonthNumPaint.setColor(
                        isFocusMonth ? mFocusMonthSundayColor : mOtherMonthSundayColor);
            } else if (i == nDays - 1) {
                mMonthNumPaint.setColor(
                        isFocusMonth ? mFocusMonthSaturdayColor : mOtherMonthSaturdayColor);
            } else {
                mMonthNumPaint.setColor(isFocusMonth ? mFocusMonthColor : mOtherMonthColor);
            }
    ...
    protected void drawDaySeparators(Canvas canvas) {
        ...
        for (int i = 1; i < mNumCells; i++) {
            int x = i * (mWidth - mPadding * 2) / mNumCells + mPadding;
            canvas.drawLine(x, 0, x, mHeight, p);
        }
        p.setStrokeWidth(DAY_SEPARATOR_WIDTH / 2);
        canvas.drawLine(0, 0, mWidth, 0, p);
        canvas.drawLine(0, mHeight, mWidth, mHeight, p);
    }
    ...
    protected void updateSelectionPositions() {
        if (mHasSelectedDay) {
            ...
            mSelectedRight =
                    (selectedPosition + 1) * (mWidth - mPadding * 2) / mNumCells + mPadding;
        }
    }
