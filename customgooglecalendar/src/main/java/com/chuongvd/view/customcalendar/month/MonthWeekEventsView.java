...
package com.chuongvd.view.customcalendar.month;
...
import com.chuongvd.view.customcalendar.Event;
import com.chuongvd.view.customcalendar.Utils;
...
public class MonthWeekEventsView extends SimpleWeekView {
    ...
    private static int TEXT_SIZE_EVENT = 8;
    private static int TEXT_SIZE_EVENT_TITLE = 8;
    private static int TEXT_SIZE_MORE_EVENTS = 8;
    ...
    private static int EVENT_MARGIN_TOP = 4;
    private static int EVENT_MARGIN_BOTTOM = 0;
    ...
    private static int BOTTOM_PADDING_MONTH_NUMBER = 8;
        ...
        public void onAnimationEnd(Animator animation) {
            synchronized (this) {
                ...
                if (mFadingIn) {
                    ...
                    mTodayAnimator =
                            ObjectAnimator.ofInt(MonthWeekEventsView.this, "animateTodayAlpha", 255,
                                    0);
    ...
    public void setEvents(List<Event> sortedEvents) {
    ...
    protected void initView() {
        ...
        if (!mInitialized) {
            ...
            if (mScale != 1) {
                EVENT_MARGIN_TOP *= mScale;
                EVENT_MARGIN_BOTTOM *= mScale;
                BOTTOM_PADDING_MONTH_NUMBER *= mScale;
        ...
        mEventExtrasPaint.setTextSize(TEXT_SIZE_MORE_EVENTS);
        ...
        if (DEBUG_LAYOUT) {
            ...
            Log.d("EXTRA", "mMonthNumPaint ascent="
                    + mMonthNumPaint.ascent()
                    + " descent="
                    + mMonthNumPaint.descent()
                    + " int height="
                    + mMonthNumHeight);
            Log.d("EXTRA", "mEventPaint ascent="
                    + mEventPaint.ascent()
                    + " descent="
                    + mEventPaint.descent()
                    + " int height="
                    ...
                    + " int ascent="
                    + mEventAscentHeight);
            Log.d("EXTRA", "mEventExtrasPaint ascent="
                    + mEventExtrasPaint.ascent()
                    + " descent="
                    + mEventExtrasPaint.descent()
                    + " int height="
                    + mExtrasHeight);
            Log.d("EXTRA", "mWeekNumPaint ascent="
    ...
    protected void drawToday(Canvas canvas) {
        ...
        r.right = computeDayLeftPosition(mTodayIndex + 1) - (int) Math.ceil(
                TODAY_HIGHLIGHT_WIDTH / 2.0f);
    ...
    protected void drawEvents(Canvas canvas) {
        ...
        int numOfEvent = 6;
        int yEventIndex = mPadding + MINI_DAY_NUMBER_TEXT_SIZE + BOTTOM_PADDING_MONTH_NUMBER;
        int eventHeight = (mHeight - 2 * mPadding - yEventIndex) / numOfEvent
                - EVENT_MARGIN_TOP
                - EVENT_MARGIN_BOTTOM;
        List<Event> drawedEvent = new ArrayList<>();
        for (int i = 0; i < numOfEvent - 1; i++) {
            int yTop = yEventIndex
                    + i * (eventHeight + EVENT_MARGIN_TOP + EVENT_MARGIN_BOTTOM)
                    + EVENT_MARGIN_TOP;
            int yBottom = yTop + eventHeight;
            boolean isMoreEvent = true;
            List<Event> eventsRow = new ArrayList<>();
            while (isMoreEvent) {
                if (drawedEvent.size() == mEvents.size()) break;
                Event eventSelected = null;
                for (Event event : mEvents) {
                    if (drawedEvent.contains(event) || isConflictEvent(event, eventsRow)) {
                        continue;
                    }
                    if (eventSelected == null) eventSelected = event;
                    if (event.endDay - event.startDay
                            > eventSelected.endDay - eventSelected.startDay) {
                        eventSelected = event;
                    }
                }
                if (eventSelected != null) {
                    eventsRow.add(eventSelected);
                    drawedEvent.add(eventSelected);
                } else {
                    isMoreEvent = false;
                }
            }
            for (Event e : eventsRow) {
                int julianDay = getFirstJulianDay();
                int start = e.startDay - julianDay;
                int end = e.endDay - julianDay + 1;
                if (start < 0) start = 0;
                if (end > mNumDays) end = mNumDays;
                int xLeft = start * (mWidth - 2 * mPadding) / mNumDays + mPadding;
                int xRight = end * (mWidth - 2 * mPadding) / mNumDays + mPadding;
                drawEvent(canvas, e, xLeft, yTop, xRight, yBottom);
            }
        }
        //
        // Draw more events
        //
        List<Event> eventsRemain = new ArrayList<>();
        for (Event event : mEvents) {
            if (drawedEvent.contains(event)) continue;
            eventsRemain.add(event);
        }
        int yTop = yEventIndex + (numOfEvent - 1) * (eventHeight
                + EVENT_MARGIN_TOP
                + EVENT_MARGIN_BOTTOM) + EVENT_MARGIN_TOP;
        int yBottom = yTop + eventHeight;
        int julianDay = getFirstJulianDay();
        int[] remain = new int[mNumDays];
        for (Event event : eventsRemain) {
            for (int i = event.startDay - julianDay; i < event.endDay - julianDay + 1; i++) {
                remain[i] += 1;
            }
        }
        for (int i = 0; i < mNumDays; i++) {
            if (remain[i] <= 0) continue;
            int xText = i * (mWidth - 2 * mPadding) / mNumDays + mPadding;
            int xTextEdge = (i + 1) * (mWidth - 2 * mPadding) / mNumDays - mPadding - xText;
            int yText = yTop + (yBottom - yTop + TEXT_SIZE_EVENT) / 2;
            drawMoreEvents(canvas, remain[i], xText, yText, xTextEdge);
        }
    }

    private boolean isConflictEvent(Event event, List<Event> eventsRow) {
        for (Event e : eventsRow) {
            if ((e.startDay < event.endDay + 1 && e.startDay >= event.startDay) || (e.endDay + 1
                    > event.startDay && e.endDay < event.endDay) || (e.startDay < event.startDay
                    && e.endDay >= event.endDay)) {
                return true;
            }
        }
        return false;
    ...
    private void drawEvent(Canvas canvas, Event event, int xLeft, int yTop, int xRight,
            int yBottom) {
        ...
        //r.top = yTop;
        //r.bottom = yBottom;
        Style boxStyle = Style.FILL_AND_STROKE;
        int color = ContextCompat.getColor(getContext(), event.color);
        mEventSquarePaint.setStyle(boxStyle);
        mEventSquarePaint.setColor(color);
        //canvas.drawRect(r, mEventSquarePaint);
        RectF rect = new RectF(xLeft, yTop, xRight, yBottom);
        canvas.drawRoundRect(rect, 1.5f, 1.5f, mEventSquarePaint);
        //
        //  Draw text Event
        //
        int xText = xLeft + BORDER_SPACE;
        int yText = yTop + (yBottom - yTop + TEXT_SIZE_EVENT) / 2;
        int textRightEdge = xRight - BORDER_SPACE;
        float avail = textRightEdge - xText;
        TextPaint textPaint = mEventPaint;
        textPaint.setTextSize(TEXT_SIZE_EVENT);
        CharSequence text =
                TextUtils.ellipsize(event.title, textPaint, avail, TextUtils.TruncateAt.END);
        canvas.drawText(text.toString(), xText, yText, textPaint);
    }
    ...
    protected int drawEvent(Canvas canvas, Event event, int x, int y, int rightEdge,
        ...
        CharSequence text =
                TextUtils.ellipsize(event.title, mEventPaint, avail, TextUtils.TruncateAt.END);
        Paint textPaint;
        if (solidBackground) {
    ...
    protected void drawMoreEvents(Canvas canvas, int remainingEvents, int x, int y, int xTextEdge) {
        String text = getContext().getResources()
                .getQuantityString(R.plurals.month_more_events, remainingEvents);
        mEventExtrasPaint.setAntiAlias(true);
        mEventExtrasPaint.setFakeBoldText(true);
        String drawText = String.format(text, remainingEvents);
        float textWidth = mEventExtrasPaint.measureText(drawText);
        canvas.drawText(drawText, x + xTextEdge - textWidth, y, mEventExtrasPaint);
        mEventExtrasPaint.setFakeBoldText(false);
    }
    ...
    //        if (hover != null && (mLastHoverTime == null
    //                || Time.compare(hover, mLastHoverTime) != 0)) {
    //            Long millis = hover.toMillis(true);
    //            String date =
    //                    Utils.formatDateRange(context, millis, millis, DateUtils.FORMAT_SHOW_DATE);
    //            AccessibilityEvent accessEvent = AccessibilityEvent.obtain(
