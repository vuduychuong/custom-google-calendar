...
public class Event implements Cloneable {
    ...
    private static void doComputePositions(ArrayList<Event> eventsList, long minimumDurationMillis,
        ...
        for (Event event : eventsList) {
            ...
            if (!doAlldayEvents) {
                colMask = removeNonAlldayActiveEvents(event, activeList.iterator(),
    ...
    private static long removeNonAlldayActiveEvents(Event event, Iterator<Event> iter,
            long minDurationMillis, long colMask) {
    ...
    public final boolean intersects(int julianDay, int startMinute, int endMinute) {
        ...
        if (endDay == julianDay) {
            ...
            if (endTime == startMinute && (startTime != endTime || startDay != endDay)) {
                return false;
            }
        }
    ...
    public static ArrayList<Event> fakeDumpListEvent() {
        ArrayList<Event> events = new ArrayList<>();
        for (int i = 0; i < 5; i++) {
            Event event = Event.newInstance();
            event.title = "ダミーダミダミーダミダ " + i;
            event.color = R.color.event_center;
            int julianMonday = Utils.getJulianMondayFromWeeksSinceEpoch(2522);
            event.startDay = julianMonday - 1;
            event.endDay = event.startDay + 10 * i;
            events.add(event);
        }

        int julianMonday = Utils.getJulianMondayFromWeeksSinceEpoch(2523);
        Event event2 = Event.newInstance();
        event2.title = "ダミーダミダミーダミダ 3";
        event2.color = R.color.event_background;
        event2.startDay = julianMonday + 2;
        event2.endDay = event2.startDay + 3;
        events.add(event2);

        Event event1 = Event.newInstance();
        event1.title = "ダミーダミダミーダミダ ";
        event1.color = R.color.month_today_number;
        event1.startDay = julianMonday;
        event1.endDay = event1.startDay + 2;
        events.add(event1);

        Event event3 = Event.newInstance();
        event3.title = "ダミーダミダミーダミダ ";
        event3.color = R.color.month_today_number;
        event3.startDay = julianMonday + 1;
